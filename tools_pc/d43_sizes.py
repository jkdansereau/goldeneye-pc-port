...
for r in rows:
    ...
    stats.append((base, D, B_pc, G_in, 16*Kslack, P_final))
    nfiles += 1

print("files:", nfiles)
worst = sorted(stats, key=lambda s: -s[5])[:15]
for b, D, Bp, Gi, K, Pf in worst:
    flag = "OVER" if Pf > R_BUF else "    "
    print("%-22s N64=%7X  B_pc=%7X  G_in=%7X  Kslack=%7X  P_final=%8X  %s (R=%X)"
          % (b, D, Bp, Gi, K, Pf, flag, R_BUF))
