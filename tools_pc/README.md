...
| `bundle-win.sh` / `bundle-linux.sh` / `bundle-mac.sh` | Package a built tree as a distributable archive (exe + licenses + `prepare-assets/`). `bundle-mac.sh` produces a signed, double-clickable `dist/GoldenEye.app` with SDL2 + libstdc++/libgcc_s bundled (Homebrew libs are not system libs) and `prepare-assets/` in `Contents/Resources/` — it cannot go in `Contents/MacOS/`, where codesign treats the directory as nested code and fails the bundle. |
| `level_sweep_mac.sh` | macOS/arm64 sweep: bare `-level_XX` boot of all 21 solo levels, crash-log detection (symbolicated via `atos`), and a `GE_PCDUMP` frame that must be non-near-black (`PIX_MIN`, default 2%), so "runs but renders nothing" is caught too. `SWEEP_SECS` (default 18). |
