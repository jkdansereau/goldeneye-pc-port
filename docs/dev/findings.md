...
| D217 | **1P weapon viewmodel + third-person Bond model textures render garbled — wrong/stale palette on model-DL CI textures (M-84, user QA; surfaced by D215 now that the viewmodel draws).** BUNKER1/Facility: the PP7 viewmodel has yellow+blue scrambled texels and a solid **green** patch by the hammer; Bond's in-game **watch face** shows the same green mismatch; user reports the **third-person Bond world model** does this "sometimes" too. Signature matches the **D161 family** — a CI-format tile drawn without the expected TLUT state, so fast3d's `import_texture` indexes a **stale `rdp.palette`**. D161 fixed the explicit `G_TT_NONE` case; this looks like model DLs not emitting `gDPLoadTLUT` through the port path, or the palette segment not resolving for weapon/character geometry. **Not a D215 regression** — D215 only un-gated the DL. | **OPEN — repro pinned to FACILITY (M-85).** Clean on `-level_09` BUNKER1 (any FovScale / TextureFilter / FixMipTextures / Anisotropy). **Reproduces on `-level_34` FACILITY, ~frame 300 (~5 s in, right after the vent-drop intro cutscene hands control to the player):** the PP7 **grip is a solid dark-green block**, a **green ring** at the barrel/slide joint, and the **hand has a mottled green cast** — while the **slide/barrel upper renders correct black**. So *some* of the weapon+hand model's CI tiles decode against a wrong palette and others don't. `GE_TEXI` shows the weapon tiles do import as `fmt=2 palfmt=0x8000 palidx=0` with a loaded TLUT — `gDPLoadTLUT` **does** reach `gfx_pc.cpp` for model DLs, so "model DLs don't emit LoadTLUT" is disproven. The green-tinted **hand** (a skin texture reading a green palette entry gives exactly this look) points at a **wrong palette**, not corrupt index data. **M-85 probe** (`GE_D217` temp log in `gfx_dp_load_tlut` + the CI branch of `import_texture`, since reverted; logs in session scratchpad `d217_fac.log` / `d217_bnk.log`): (i) **user confirms it is not Facility-specific — guns on many levels, and multiple weapon types, render with wrong or missing textures**; the Facility PP7 is just a clean headless repro. (ii) GE weapon/character model DLs issue **partial-count `G_LOADTLUT` commands** — `count` = 1, 28, 53, 103, 123, 125, 172, 193, … (never `<<2`-encoded, so the decode is reading the raw count field of `gsDPLoadTLUTCmd` correctly; 256 also appears for world tiles). A `count`=125 load writes `rdp.palette[0..124]` and leaves `[125..255]` holding **whatever the previous TLUT load left** — and those stale upper entries differ per level (Facility `pal[128,200,255]=8c1b eedf f7f5` vs BUNKER1 `841f 9b15 fa53`). (iii) The weapon TLUT load **sequence + counts are identical on BUNKER1 and FACILITY** (same PP7), and the weapon CI8 tiles import with `palidx=0` + `cache=HIT` on the steady-state frames. So the delta is purely the **stale upper-palette contents** a weapon CI8 texel lands in when it indexes ≥ its palette's `count`. (iv) **`GE_D217Z` experiment — zeroing `rdp.palette[count..255]` after every partial load did NOT change the green grip/ring/hand** → the bad texels index **within** the loaded `[0..count)` range, so the *palette content bound to the grip tile is wrong*, not stale-tail overflow. (v) Best remaining theory: **`TextureCacheKey` collision / missing CI-cache invalidation on palette change** (`gfx_pc.cpp:990` — key = `{orig_addr, palette_addrs[0/1], fmt, siz, palidx, size_bytes}`, no palette *content* hash; `rdp.textures_changed` is set by `gfx_dp_load_tlut` but may not evict CI entries). Facility has **green-uniformed guards rendered right before the viewmodel**; if the grip tile takes a stale `cache=HIT` (or its `palette_addrs` collide with the guards' TLUT source) it decodes against the **guard-fatigue palette** → green grip + green-cast hand, barrel (different material/palette, imported clean) stays black. Fits "widespread, weapon- and proximity-dependent." Next: log CI-cache hit/miss + a cheap palette checksum per weapon tile on Facility; test making the CI cache key include a palette-content hash (or evicting CI entries on `textures_changed`) — **must regression-sweep all 21 levels' world CI textures**, this is a hot path. **Wants a dedicated debug session / scoped subagent.** Screenshots: session scratchpad `fac_gun_crop.png`; add to `docs/dev/GRAPHICS-BACKLOG.md`. |
| D218 | **Wide FOV exposes the per-level draw-distance / fog boundary — "blue artifacting" down long peripheral sightlines (M-84, user QA on the D211 FOV slider).** BUNKER→**Dam main tunnel** at `FovScale` ~150: geometry / backdrop past the level's fog-end distance shows at the widened frustum edges (blue = Dam's fog tint). Not a render bug — `fogLoadCurrentEnvironment` (`bgfog.c:305`) does `viSetZRange(Visibility.BlendMultiplier, Visibility.FarFog)`, so **`Visibility.FarFog` is both the far clip plane and the fog-saturation distance**; levels tune it for the stock ~60° FOV. | **Logged (M-84), fix proposed + parked.** Plan: `Video.DrawDistance` (percent, 100–400, default 100 ⇒ no-op) `#ifdef PORT` multiplier on `FarFog` at `bgfog.c:305` (+ the direct `arg0->Visibility.FarFog` reads just below, so fog stays consistent) plus `Video.DrawDistanceAutoFov` (0/1, default 1 — couples draw distance to `FovScale` unless `DrawDistance` is set explicitly). Clamp effective multiplier ≲2.5× (small znear ⇒ far-field z-fighting risk). |
