...
| D231 | **QoL batch: mute-on-focus-loss + screenshot-hotkey productization (M-105, roadmap Tier 3, port-only).** Two cheap remaster-parity items. **(1) Mute on focus loss** — `Audio.MuteOnFocusLoss` (default 1): the host event pump (`video.c`) calls `audioHandleFocus()` on `SDL_WINDOWEVENT_FOCUS_LOST/GAINED`; while muted, mixed blocks are dropped instead of queued in the `osAiSetNextBuffer` path (`port/src/audio.c`). The game's AI regulator keeps producing and the queue is empty by the time focus returns, so audio resumes with fresh samples rather than a burst of stale ones. **Pause-on-focus-loss was considered and deferred:** pausing the sim from the port layer interacts with the AI feedback loop + scheduler (D204 family) and needs its own design; mute alone kills the "blaring audio on alt-tab" half of the complaint (drive-by deaths while unfocused remain — same as the N64, which had no such concept). **(2) Screenshot hotkey** — productizes the pre-existing F12 debug dump (`videoPreSwapCapture`): `Video.ScreenshotKey` (a single SDL scancode name in the D214 style, default "F12", parsed via `SDL_GetScancodeFromName`, unknown names fall back to F12 with a warning), `Video.ScreenshotDir` (default `screenshots/`), timestamped filenames `screenshot_YYYYMMDD_HHMMSS.ppm` with an `_N` suffix on same-second collisions. The key check is now scancode-based (layout-independent) instead of the old keysym compare; the F12 default reproduces prior behaviour exactly. | LANDED (M-105, port-only: `port/src/audio.c`, `port/src/video.c`, `port/include/audio.h`). Build green; smoke run confirmed the new keys migrate into an existing `data/ge007.ini` and the game boots clean. **Owed:** human playtest — alt-tab mid-level (audio mutes/unmutes, no crash), screenshot key in-game (PPM lands in `screenshots/` with a timestamped name), one custom key + dir via ini. |
