...
| D230 | **In-level music quality: some instruments on certain tracks sound wrong, and audio occasionally garbles in general play (M-105, user reports; the elevator track is named specifically).** Filed to give the post-D77 residual its own label — D77 (in-level music *absence*) is RESOLVED (PR #42, missing `return` in `sub_GAME_7F0C0BF0` zeroed the seqp volume on the only in-level trigger path), so "music doesn't play" claims in tracked docs are stale; what actually remains is this quality gap. The mixer layer and the trigger layer are both proven working by every track that sounds right, so the suspect neighborhood is the seq/instrument-bank decode path (`csplayer.c`/`synthesizer.c`, bank loading), not `port/` audio plumbing. | **OPEN — filed M-105, not yet investigated.** First step is a repro: which track(s)/instrument(s) are wrong, and is it every playthrough or intermittent — that bears on whether it's a decode bug (deterministic) or a live mixing/voice-stealing issue (D147/D152/D202 family). |
...
| D230 | **In-level music quality: a bass instrument sounds corrupted on PC, correct on N64 reference (M-106, user QA report).** Repro is deterministic, not intermittent: Control's elevator track and its regular in-level track both carry the bad instrument throughout; Cavern's elevator track has it too; Runway's track is otherwise fine but the same instrument enters ~5–7s after the track starts and sounds bad from then on. User confirmed against original N64 audio that a bass instrument belongs there on Runway — this is **not** a wrong-instrument/wrong-bank selection (rules out the D77 trigger-path family and the seqp-volume fix's neighborhood); the instrument choice is correct, only its *sound* is wrong on PC. Deterministic-per-track (not per-playthrough) points at a static decode/synthesis bug for this instrument's wavetable/ADPCM data — most likely low-frequency-specific (pitch-shift/downsample artifacting, a wrap/loop-point error, or a clipping/gain issue that only bites bass-range samples) — rather than a live mixing/voice-stealing issue (D147/D152/D202 family), which would be intermittent. Scope to the synth/instrument-bank decode path (`csplayer.c`/`synthesizer.c` or bank loading), not the mixer (proven sound by every correctly-playing track) or the D77 trigger path (proven sound — music plays, this is a quality-only residual on specific instruments). | OPEN — not investigated. Repro: Control (elevator + regular track, throughout), Cavern (elevator, throughout), Runway (~5–7s in). Next: isolate the instrument/patch id these three tracks share and diff its wavetable/ADPCM decode against a known-good (non-bass) instrument on the same tracks. |
...
| D235 | **Facility: wrong textures "now" — user reports a regression (M-106, user QA report).** Prime suspect: the recent texture work (#47 texpool alignment, #49 CI-cache palette key, D228 split). Bisect against pre-D217-fix build first. Logged in `GRAPHICS-BACKLOG.md`. | OPEN — needs which-surfaces + screenshot vs older build |
