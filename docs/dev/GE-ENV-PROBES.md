...
| `GE_D193A` | `src/game/chr.c` (`chrTick` → `d193aSample`, `#ifdef PORT`) | Per-wall-second per-ticked-chr locomotion telemetry (D193): `chr=` chrnum, `act=` ACT_TYPE, `speed=` world-units travelled/s, `gtd=` g_GlobalTimerDelta, then the root-motion scalars `scale` / `xlscale` (anim_translation_scale) / `animrate` / `playspeed` / `mspeed` (model->speed) / `endframe` / `anim` ptr. `GE_D193A_CHR=<n>` restricts to one chrnum. M-80: patrolling guards (act=14) travel ~60–99 u/s on the port; gtd=2.0, playspeed=1.0, scale=0.1, xlscale=1.0, animrate=0.0 (default; only set on blended transitions) — all nominal, so D193 is not in these inputs. Needs an N64-decomp comparison run. | **live** (D193) |
